...
function sys_Sound:process(e, dt)
	...
	sound.source:setPosition(e.Position.x / world.SOUND_POSITION_SCALE, e.Position.y / world.SOUND_POSITION_SCALE, 0)

	if e.Velocity then
		sound.source:setVelocity(
			e.Velocity.x / world.SOUND_POSITION_SCALE,
			e.Velocity.y / world.SOUND_POSITION_SCALE,
			0
		)
	end
	...
	if not sound.source:isLooping() and not sound.source:isPlaying() and sound.played then
