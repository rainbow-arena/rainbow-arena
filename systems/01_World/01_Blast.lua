...
--- Constants ---
local DEFAULT_BLAST_COLOR = { 1, 0.38, 0 }
--- ==== ---
...
local function update_blast(e, world, dt)
	...
	if blast.radius and blast.duration and blast.func then
		...
		for affected in pairs(world.hash:get_objects_in_aabb(circle.aabb(current_radius, e.Position.x, e.Position.y))) do
...
local function draw_blast(e)
	...
	love.graphics.setColor(color[1], color[2], color[3], fade)
	love.graphics.circle("line", e.Position.x, e.Position.y, cradius)

	love.graphics.setColor(color[1], color[2], color[3], fade * 0.25)
